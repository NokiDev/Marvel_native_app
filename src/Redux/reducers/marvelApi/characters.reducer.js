...
const charactersReducers = (state = initialState, action) => {
    switch (action.type) {
            ...
            case characterActions.FETCH_CHARACTER_BY_ID_SUCCESS:
                const array = [...state.array, action.payload.character.id].filter((id) => state.array.includes(id) === false)
                let char = {}
                char[action.payload.character[0].id] = action.payload.character[0]
